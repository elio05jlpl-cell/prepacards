---
title: Apprendre le vocabulaire espagnol : la méthode | PrépaCards
description: Listes oubliées en trois jours, mots reconnus mais jamais produits : pourquoi le vocabulaire espagnol ne rentre pas, et le protocole qui le fait tenir.
date: 2026-09-30
slug: apprendre-le-vocabulaire-espagnol
---

# Apprendre le vocabulaire espagnol : la méthode

Vous apprenez une liste de trente mots le dimanche. Le mardi, il en reste dix.
Le mois suivant, vous ne sauriez pas dire lesquels vous aviez vus. Ce n'est pas
un problème de mémoire — c'est que la liste est un mauvais format.

## Pourquoi les listes ne tiennent pas

Trois raisons, qui se cumulent.

**L'effet de position.** Dans une liste de trente mots, vous retenez les
premiers et les derniers. Le milieu s'évapore, et il s'évapore toujours au même
endroit.

**Le contexte trompeur.** Réviser une liste thématique — le travail, la santé —
vous donne un contexte qui souffle les réponses. Vous croyez savoir *desempleo*
parce que vous êtes dans la liste « économie ». Hors contexte, le mot ne vient
pas.

**L'absence d'espacement.** Un mot vu une fois le dimanche n'est pas revu avant
le dimanche suivant, si tant est qu'il le soit. La courbe de l'oubli fait son
travail entre les deux.
[Pourquoi la LV2 décroche plus vite que la LV1 →](/blog/lv2-en-prepa/)

## Le sens qui compte : français → espagnol

C'est la décision la plus importante, et presque personne ne la prend
consciemment.

Une carte **espagnol → français** entraîne à *reconnaître*. Une carte
**français → espagnol** entraîne à *produire*.

Toutes les épreuves demandent la production : l'essai, le thème, l'expression
orale. Un mot que vous ne reconnaissez qu'à la lecture ne sortira pas quand il
faudra l'écrire.

Travaillez donc dans le sens difficile. C'est plus lent au début, et c'est le
seul qui serve.

## Ce qu'il faut mettre sur une carte

**Des expressions plutôt que des mots nus.** *Hacer frente a* — faire face à —
se retient mieux et se réutilise mieux que *frente* isolé, parce qu'elle porte
sa construction avec elle.

**Le verbe avec sa préposition.** *Soñar **con***, *pensar **en***, *tratar
**de***. C'est ce qui se perd en premier, et ce qui se voit le plus dans une
copie.

**Le genre avec le nom**, quand il n'est pas devinable. *El problema*, *la
mano* : deux pièges classiques qu'une carte règle en trois répétitions.

**Les faux amis, par deux.** *Embarazada* ne veut pas dire embarrassée. Une
carte pour le vrai sens, une carte pour le mot français correspondant —
sinon la confusion reste.

**Le vocabulaire de vos propres textes.** Les mots rencontrés dans un article
travaillé en cours valent plus qu'une liste générique : vous avez déjà un
contexte et une phrase pour eux.

## Le protocole, concrètement

**Quinze à vingt mots nouveaux par jour, tous les jours.** Pas cinquante le
dimanche. Une langue se maintient par contacts brefs et répétés.
[Le calcul complet du nombre de cartes →](/blog/combien-de-cartes-par-jour/)

**Mélangez les thèmes à la révision.** Constituez vos paquets par thème si vous
voulez, mais révisez-les ensemble : c'est le mélange qui supprime le contexte
qui souffle.

**Dites la réponse à voix haute avant de retourner la carte.** C'est
inconfortable et c'est tout l'intérêt : l'hésitation s'entend, et l'illusion de
savoir tombe immédiatement.
[Pourquoi la récitation change tout →](/blog/reviser-a-voix-haute/)

**Utilisez les temps morts.** Dix minutes dans les transports, dix minutes avant
de dormir. Quatre créneaux de dix minutes par jour font presque cinq heures par
mois — plus que votre volume de cours.
[Préparer la khôlle d'espagnol →](/blog/preparer-une-kholle/)

## L'accentuation, qui coûte des points bêtement

*Está* et *esta*, *él* et *el*, *sí* et *si* : l'accent n'est pas décoratif, il
change le mot. Si vous écrivez vos cartes sans accents, vous apprendrez une
orthographe fausse et vous la reproduirez en copie.

Même remarque pour le *ñ*. Écrivez-le correctement dès la première carte : une
graphie mémorisée de travers est plus difficile à corriger qu'à apprendre.

## En pratique

PrépaCards vérifie la prononciation **en espagnol** au micro : vous dites le
mot, l'application transcrit ce qu'elle a entendu et compare à la réponse
attendue. Un mot marmonné ne passe pas, et la carte revient. La transcription
tourne sur votre ordinateur, hors ligne — donc dans le train aussi.

Les paquets fournis ne couvrent que l'anglais ECG : pour l'espagnol, vous créez
vos cartes, avec une traduction proposée à la saisie pour aller plus vite.

<div class="encart">
  <p>Gratuit pour Windows 10 et 11, avec un compte gratuit.
  <a href="/telecharger/">Télécharger</a> ·
  <a href="/blog/lv2-en-prepa/">Tenir sa LV2 en prépa</a></p>
</div>
